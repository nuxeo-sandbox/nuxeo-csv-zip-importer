...
public class CSVZipImporter extends AbstractFileImporter {
            ...
            try (Reader reader = new InputStreamReader(zip.getInputStream(index));
                ...
                for (CSVRecord csvRecord : csvParser) {
                    ...
                    for (String fname : stringValues.keySet()) {
                        ...
                        } else if (fname.equalsIgnoreCase("nxtags:tags")) {
                            DocumentHelper.setProperty(targetDoc.getCoreSession(), targetDoc, fname, stringValue);
                        } else if (fname.contains(":")) {
                            if (targetDocType.hasField(fname)) {
                                field = targetDocType.getField(fname);
                                usePrefix = true;
                            } else if (fname.equals("file:content")) {
                                field = targetDocType.getField("content");
                                usePrefix = true;
                            }
                        } else if (fname.contains(".")) {
    ...
    protected Serializable getFieldValue(Field field, String stringValue, ZipFile zip, CoreSession documentManager) {
        Serializable fieldValue = null;
        Type type = field.getType();
        if (type.isSimpleType()) {
            if (type instanceof SimpleTypeImpl) {
                // return null when empty value for a directory field
                if (type.getObjectResolver() instanceof DirectoryEntryResolver && stringValue.isEmpty()) {
                    return null;
                }
            ...
            if (type instanceof StringType) {
                ...
                try {
                    ...
                    if (stringValue.length() == 10) {
                        date = new SimpleDateFormat(DATE_FORMAT).parse(stringValue);
            ...
            if (StringUtils.isNotBlank(stringValue)) {
                fieldValue = (Serializable) stringValue.split("\\|");
            } else {
                // Empty cell
                return null;
            }
